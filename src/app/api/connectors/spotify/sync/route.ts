...
export async function POST(req: NextRequest) {
  ...
  const spotify = statuses.find(
    (c) => c.connectorType === "spotify" && c.status === "connected",
  );
  ...
  if (isSyncRateLimited(spotify.lastSync)) {
    return connectorError(
      "RATE_LIMITED",
  ...
  const jobId = enqueueJob("connector_sync", { ownerKey, connectorId: spotify.id });
  if (!jobId) {
    return connectorError(
      "ALREADY_SYNCING",
      "A sync is already in progress.",
