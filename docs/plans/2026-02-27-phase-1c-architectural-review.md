...
**Date**: 2026-02-27
**Status**: Review v3 — all targeted fixes applied, ready for implementation plan rewrite
...
**How per-section hashing works:**

Each section type has relevant fact categories defined in `SECTION_FACT_CATEGORIES` (e.g., `bio → [identity, interest]`, `skills → [skill]`). The `facts_hash` stored in `section_copy_state` is computed from ONLY the publishable facts in that section type's relevant categories:

```typescript
// Compute per-section facts hash
function computeSectionFactsHash(
  ...
  return computeHash(JSON.stringify(relevant.map(f => ({
    id: f.id, category: f.category, key: f.key, value: f.value, visibility: f.visibility,
  }))));
}
```
...
**Problem:** The `personalizationPending` in-memory flag is fragile (lost on restart, doesn't work multi-instance). Making it durable adds a DB column + write on every synthesis start/end + read on every SSE tick — significant complexity for a visual nicety.

**Decision: Drop it entirely.** The copy upgrade happens silently:

1. `generate_page` writes optimistic (deterministic) config via `upsertDraft()` immediately
2. Fire-and-forget synthesis runs in background
3. Synthesis writes result to `section_copy_state`
4. On next SSE tick (1s interval), `mergeActiveSectionCopy()` picks up the new active copy
5. Config hash changes → client receives updated config → renders new text

The user sees the text swap within ~1-2 seconds of synthesis completing. No shimmer, no loading state, no new SSE fields. The preview just "gets better" naturally.

**What's removed:**
- `personalizationPending` field from `PreviewEvent` type
- In-memory `Map<string, boolean>` in web process
- Shimmer CSS and client-side handling (T15 from Part 3)
- Any SSE payload extension

**If shimmer is wanted later:** Add a `pending_regen` boolean column to `section_copy_state` (set true on synthesis start, false on completion). `mergeActiveSectionCopy()` can read it and include it in the returned config as metadata. But YAGNI for Phase 1c.
...
function acceptProposal(proposalId: string): { ok: boolean; error?: string } {
  ...
  if (activeState && activeState.approvedAt > proposal.createdAt) {
    markProposalStale(proposalId);
