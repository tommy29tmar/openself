...
- Heartbeat Separation & Deep Fact Gate (2026-03-10): Three-tier heartbeat architecture — global housekeeping (once per scheduler tick, table-wide SQL), light heartbeat (daily, owner-scoped deterministic, no LLM), deep heartbeat (weekly, LLM-only). Deep gated by `DEEP_HEARTBEAT_MIN_FACTS` (25) checked at both scheduling and execution time with `cognitiveOwnerKey` for canonical-mode correctness. Non-terminal outcomes (below_threshold, budget_exceeded) don't record `heartbeat_runs` — weekly window stays open for retry. Both substeps (conformity + coherence) must complete for run recording (`&&` logic). `ownerDay` captured at enqueue time in scheduler payload to prevent midnight-crossing misclassification. 6 rounds Codex cross-model validation.
- Extended Thinking (2026-03-10): Opt-in chain-of-thought reasoning via `AI_THINKING_BUDGET` env var (default 8000 tokens). Activates Anthropic `thinking.type: "enabled"` with `budgetTokens` when >= 1024. Reasoning output logged to `[thinking]` for agent behavior debugging and system prompt fine-tuning.
...
| Heartbeat engine | Done | Three-tier (global housekeeping + light daily + deep weekly). Light: deterministic, no LLM. Deep: LLM-dependent, gated by min 25 facts, retry-friendly (non-terminal outcomes don't block weekly window). |
| Heartbeat scheduler | Done | Auto-enqueues heartbeat jobs for active owners every 15 min. Light: daily at 3 AM owner tz (catch-up). Deep: Sunday 3 AM (catch-up) + Monday recovery. Fact-count gate. ownerDay captured at enqueue. Anti-overlap lock. ISO-week DST-safe. |
